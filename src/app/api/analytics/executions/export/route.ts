...
export async function GET(request: NextRequest) {
  try {
    ...
    const dirLabel = (d: string) =>
      d === "LONG" ? "做多" : d === "SHORT" ? "做空" : "—";
    const accLabel = (a: string | null) =>
      a === "CORRECT" ? "正确" : a === "WRONG" ? "有误" : "";
    ...
    const csv =
      "﻿" + allRows.map((row) => row.map(csvEscape).join(",")).join("\r\n");

    const today = new Date().toISOString().split("T")[0]!;
    return new NextResponse(csv, {
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="mnq-executions-${today}.csv"`,
      },
    });
  } catch (error) {
