...
} from "~/components/weekly/WeeklyReportClient";
...
export default async function WeeklyDetailPage({ params }: PageProps) {
  ...
  const timeframeStatsMap = new Map<
  ...
  for (const session of sessions) {
    ...
    if (session.planFollowed != null) {
      planSum += session.planFollowed;
      planN++;
    }
    if (session.emotionRating != null) {
      emotionSum += session.emotionRating;
      emotionN++;
    }
    if (session.focusRating != null) {
      focusSum += session.focusRating;
      focusN++;
    }
      ...
      if (setup.status === "EXECUTED") {
        ...
        for (const ex of setup.executions) {
          ...
          trades.push({
            ...
            executionGrade:
              (ex.executionGrade as "A" | "B" | "C" | "D" | null) ?? null,
            strategy: setup.strategy ?? null,
    ...
    if (plan) {
      ...
      for (const { key, raw } of MNQ_SEGS) {
        ...
        for (const opp of parseMnqOpps(raw)) {
          const decisionTimeframe = isMnqDecisionTimeframe(
            opp.decisionTimeframe,
          )
            ? opp.decisionTimeframe
            : null;
          if (opp.captured === false) {
            ...
            const riskPts = opp.missedRiskPts
              ? parseFloat(opp.missedRiskPts)
              : null;
            const returnPts = opp.missedReturnPts
              ? parseFloat(opp.missedReturnPts)
              : null;
            mnqMissed.push({
              ...
              returnPts:
                returnPts !== null && !isNaN(returnPts) ? returnPts : null,
          ...
          if (opp.tradeDirection !== null) {
            ...
            const targetPx = opp.targetPrice
              ? parseFloat(opp.targetPrice)
              : null;
            const riskPts = opp.plannedRiskPts
              ? parseFloat(opp.plannedRiskPts)
              : null;
            const maePts = opp.maxDrawdownPts
              ? parseFloat(opp.maxDrawdownPts)
              : null;
            const mfePts = opp.maxFavorablePts
              ? parseFloat(opp.maxFavorablePts)
              : null;
            trades.push({
  ...
  const winRate =
    mnqTradeCount > 0 ? Math.round((mnqWinCount / mnqTradeCount) * 100) : null;
  const planScore = planN > 0 ? Math.round((planSum / planN) * 20) : null;
  const emotionScore =
    emotionN > 0 ? Math.round((emotionSum / emotionN) * 20) : null;
  ...
  const systemScoreTotal =
    scoreDims.length > 0
      ? Math.round(
          scoreDims.reduce((s, d) => s + d.score, 0) / scoreDims.length,
        )
      : null;
  ...
  const uniqueMondays = [
    ...new Set(
      allSessions.map((s) => getMondayOf(s.date).toISOString().split("T")[0]!),
    ),
  ].sort();
  const currentIdx = uniqueMondays.indexOf(weekStartStr);
  const prevWeek =
    currentIdx > 0 ? (uniqueMondays[currentIdx - 1] ?? null) : null;
  const nextWeek =
