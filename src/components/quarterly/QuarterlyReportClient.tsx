...
export interface MonthSummary {
  ...
  equity: number[]      // relative cumulative MNQ PnL within month
}

export interface RecurringLesson {
  text: string
  occurredIn: string[]     // yearMonth strings ["2026-04", "2026-05"]
  occurredLabels: string[] // short labels ["Apr", "May"]
  severity: "high" | "medium" | "low"
}

export interface PersistentIssue {
  issue: string
  monthsAffected: string[]  // yearMonth strings
  allMonthYMs: string[]
  allMonthLabels: string[]
}

export interface QuarterlyTradeRecord {
  id: string
  yearMonth: string
  monthLabel: string
  day: string
...
export interface QuarterlyMnqMissedRecord {
  id: string
  yearMonth: string
  monthLabel: string
  day: string
...
interface Props {
  ...
  quarterEquity: number[]
  monthBoundaryIndices: number[]
  months: MonthSummary[]
  recurringLessons: RecurringLesson[]
  persistentIssues: PersistentIssue[]
  allTrades: QuarterlyTradeRecord[]
  mnqMissed: QuarterlyMnqMissedRecord[]
...
function Sec({ children, sub }: { children: React.ReactNode; sub?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
      ...
      <span style={{ fontSize: 11.5, fontWeight: 600, color: C.textMid, letterSpacing: "0.07em" }}>{children}</span>
...
// ── Quarter equity curve ──────────────────────────────────────────────────────

function QuarterEquityCurve({ data, boundaries }: { data: number[]; boundaries: number[] }) {
  const W = 1100, H = 160
  ...
  const spread = Math.max(Math.abs(Math.max(...data)), Math.abs(Math.min(...data)), 1)
  const min = Math.min(...data) - spread * 0.05 - 10
  const max = Math.max(...data) + spread * 0.05 + 10
  const range = max - min
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H, display: "block" }}>
      <defs>
        <linearGradient id="qEqGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      {boundaries.slice(0, -1).map((idx, i) => (
        <line key={i} x1={xScale(idx)} y1={PAD.t} x2={xScale(idx)} y2={H - PAD.b}
...
function MonthSpark({ data, color }: { data: number[]; color: string }) {
  const W = 200, H = 36
...
function MonthCard({ m }: { m: MonthSummary }) {
  const [hov, setHov] = useState(false)
  const pnlColor = m.pnl >= 0 ? C.green : C.red
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px 12px", borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ fontSize: 10, color: C.textDim, marginTop: 3, fontFamily: "DM Mono, monospace" }}>{m.range}</div>
      </div>

      {/* sparkline */}
      <div style={{ padding: "8px 8px 0" }}>
        <MonthSpark data={m.equity} color={pnlColor} />
      </div>

      {/* metrics */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", padding: "4px 8px 12px", borderBottom: `1px solid ${C.border}` }}>
        {[
          { l: "交易", v: m.trades, c: C.text },
          { l: "胜率", v: m.trades > 0 ? `${m.winRate}%` : "—", c: m.winRate >= 60 ? C.green : C.text },
          { l: "盈/亏", v: m.trades > 0 ? `${m.wins}/${m.trades - m.wins}` : "—", c: C.text },
        ].map((s) => (
      ...
      {/* regimes */}
      {(m.regimes.TREND + m.regimes.CHOP + m.regimes.RANGE) > 0 && (
        <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <div style={{ display: "flex", height: 6, borderRadius: 3, overflow: "hidden", background: C.border }}>
            {(["TREND", "CHOP", "RANGE"] as const).map((type) =>
              m.regimes[type] > 0 ? (
                <div key={type} style={{ flex: m.regimes[type], background: REGIME_COLORS[type] }}
                  title={`${REGIME_LABELS[type]} ${m.regimes[type]}天`} />
              ) : null
            )}
          </div>
            ...
            {(["TREND", "CHOP", "RANGE"] as const).map((type) =>
              m.regimes[type] > 0 ? (
                <span key={type} style={{ fontSize: 9.5, color: REGIME_COLORS[type], display: "flex", alignItems: "center", gap: 3 }}>
                  ...
                  <span style={{ fontFamily: "DM Mono, monospace" }}>{m.regimes[type]}</span>
                </span>
              ) : null
            )}
          </div>
      ...
      {m.insight && (
        <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}` }}>
          <p style={{
            fontSize: 11.5, lineHeight: 1.6, color: C.textMid,
...
export function QuarterlyReportClient({
  yearQuarter, quarterLabel, range,
  prevQuarter, nextQuarter,
  totalPnL, totalTrades, winCount, profitableMonths, avgScore,
  quarterEquity, monthBoundaryIndices,
  months, recurringLessons, persistentIssues, allTrades, mnqMissed,
  ...
  const handleSave = useCallback(async () => {
    setSaving(true)
    try {
      const res = await fetch(`/api/quarterly-reports/${yearQuarter}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ quarterInsight: insight || null }),
      })
  ...
  }, [yearQuarter, insight, router])

  const TD: React.CSSProperties = { padding: "10px", borderBottom: `1px solid ${C.border}` }

  // summary strip stats
  const tradePnls = allTrades.filter((t) => t.pnl !== null).map((t) => t.pnl!)
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", height: 52, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            >
              <Save size={13} />
              {saving ? "保存中..." : saved ? "已保存" : "保存季报"}
            </button>
            <div style={{ display: "flex", gap: 6 }}>
              <NavBtn onClick={() => prevQuarter && router.push(`/quarterly/${prevQuarter}`)} disabled={!prevQuarter}>‹</NavBtn>
              <NavBtn onClick={() => nextQuarter && router.push(`/quarterly/${nextQuarter}`)} disabled={!nextQuarter}>›</NavBtn>
            </div>
          </div>
        </div>
      </div>
        ...
        {months.length === 0 ? (
          ...
          <>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 18 }}>
                ...
                <div style={{ display: "flex", gap: 24 }}>
                  {[
                    ...
                    { l: "盈利月", v: `${profitableMonths}/${months.length}`, c: profitableMonths >= Math.ceil(months.length / 2) ? C.green : C.amber },
              ...
              <Card>
                <QuarterEquityCurve data={quarterEquity} boundaries={monthBoundaryIndices} />
              </Card>
            </div>

            {/* ── 3-month grid ── */}
            <Sec sub="点击「查看完整月报」跳转详情">本季各月</Sec>
            <div style={{
              display: "grid",
              gridTemplateColumns: `repeat(${months.length}, 1fr)`,
              gap: 12, marginBottom: 24,
            }}>
              {months.map((m) => <MonthCard key={m.yearMonth} m={m} />)}
            </div>
                        ...
                        return (
                          <div key={i} style={{
                            ...
                            <div style={{ marginTop: 6, fontSize: 10, color: sevColor, display: "flex", alignItems: "center", gap: 4 }}>
                              <span>⚠</span>
                              <span>已重复出现 {l.occurredIn.length} 个月，需重点强化</span>
                            </div>
                          </div>
                        )
                      })}
                        ...
                        <div key={i} style={{ padding: "12px 14px", background: C.surface2, borderRadius: 10, border: `1px solid ${C.border}` }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                            ...
                            <span style={{ fontSize: 10.5, color: C.red, fontFamily: "DM Mono, monospace" }}>
                              {p.monthsAffected.length}/{p.allMonthYMs.length} 月受影响
                            </span>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ fontSize: 10, color: C.textDim, width: 50 }}>影响月</span>
                            <div style={{ display: "flex", gap: 4, flex: 1 }}>
                              {p.allMonthYMs.map((ym, j) => {
                                const affected = p.monthsAffected.includes(ym)
                                return (
                                  <div key={ym} title={p.allMonthLabels[j]} style={{
            ...
            {allTrades.length > 0 && (
              <Card style={{ marginBottom: 16 }}>
                <Sec sub={`${allTrades.length} 笔 · 按月分组`}>季度交易明细</Sec>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                        {["编号", "时间", "品种", "方向", "入场价", "出场价", "盈亏", "执行", "策略", "子策略", "进入方式", "备注"].map((h) => (
                    ...
                    <tbody>
                      {months.map((month) => {
                        const mTrades = allTrades.filter((t) => t.yearMonth === month.yearMonth)
                        if (mTrades.length === 0) return null
                        const mPnl = mTrades.filter((t) => t.pnl !== null).reduce((s, t) => s + t.pnl!, 0)
                        return (
                          <Fragment key={`mo-${month.yearMonth}`}>
                            <tr style={{ background: `${C.amber}0d` }}>
                              <td colSpan={12} style={{ padding: "8px 10px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 11 }}>
                                  ...
                                  <span style={{ color: C.textDim }}>{month.range}</span>
                                  <span style={{ flex: 1 }} />
                                  <span style={{ color: C.textDim, fontFamily: "DM Mono, monospace" }}>{mTrades.length} 笔</span>
                                  <span style={{ color: mPnl >= 0 ? C.green : C.red, fontWeight: 600, fontFamily: "DM Mono, monospace" }}>
                                    {fmt(Math.round(mPnl))}
                                  </span>
                                </div>
                              </td>
                            ...
                            {mTrades.map((t) => {
                              ...
                              const dirColor = t.direction === "LONG" ? C.green : C.red
                              return (
                                <tr key={t.id}
                                ...
                                >
                                  <td style={{ ...TD, color: C.textDim, fontSize: 10.5, fontFamily: "DM Mono, monospace" }}>{t.id}</td>
                                  <td style={{ ...TD, color: C.textMid, fontFamily: "DM Mono, monospace" }}>
                                    {t.day}
                                    {t.time && <span style={{ color: C.textDim, marginLeft: 6, fontSize: 10.5 }}>{t.time}</span>}
                                  </td>
                                  <td style={{ ...TD, color: C.text, fontWeight: 500, fontFamily: "DM Mono, monospace" }}>{t.symbol}</td>
                                  <td style={{ ...TD, color: dirColor, fontWeight: 500, fontSize: 11.5 }}>
                                    {t.direction === "LONG" ? "↑ 多" : "↓ 空"}
                                  </td>
                                  <td style={{ ...TD, color: C.textMid, fontFamily: "DM Mono, monospace", fontSize: 11 }}>
                                    {t.entryPrice != null ? t.entryPrice.toLocaleString() : "—"}
                                  </td>
                                  <td style={{ ...TD, color: C.textMid, fontFamily: "DM Mono, monospace", fontSize: 11 }}>
                                    {t.exitPrice != null ? t.exitPrice.toLocaleString() : "—"}
                                  </td>
                                  <td style={{
                                    ...TD, color: pnlColor, fontWeight: 600, fontSize: 14,
                                    fontFamily: "DM Mono, monospace",
                                    textShadow: t.pnl !== null && Math.abs(t.pnl) > 500 ? `0 0 10px ${pnlColor}88` : "none",
                                  }}>
                                  ...
                                  {/* 策略 */}
                                  <td style={{ ...TD, fontSize: 11, color: "oklch(0.72 0.15 240)", maxWidth: 140, whiteSpace: "nowrap" }}>
                                    {t.strategy ?? <span style={{ color: C.textDim }}>—</span>}
                                  </td>
                                  {/* 子策略 */}
                                  <td style={{ ...TD, fontSize: 11, color: C.textDim, maxWidth: 140, whiteSpace: "nowrap" }}>
                                    {t.tradeTypeName ?? "—"}
                                  </td>
                                  {/* 进入方式 */}
                                  ...
                                  {/* 备注 */}
                                  <td style={{ ...TD, fontSize: 11, color: C.textDim, maxWidth: 180 }}>
                                    {t.notes ?? ""}
                                  </td>
                                </tr>
                              )
            ...
            {mnqMissed.length > 0 && (
              <Card style={{ marginBottom: 16 }}>
                <Sec sub={`${mnqMissed.length} 笔 · 按月分组`}>季度错过 MNQ 机会</Sec>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <tbody>
                      {months.map((month) => {
                        const mMissed = mnqMissed.filter((m) => m.yearMonth === month.yearMonth)
                        if (mMissed.length === 0) return null
                        return (
                          <Fragment key={`mo-miss-${month.yearMonth}`}>
                            <tr style={{ background: `${C.red}0a` }}>
                              <td colSpan={11} style={{ padding: "8px 10px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 11 }}>
                                  ...
                                  <span style={{ color: C.textDim }}>{month.range}</span>
                                  <span style={{ flex: 1 }} />
                                  <span style={{ color: C.textDim, fontFamily: "DM Mono, monospace" }}>{mMissed.length} 笔</span>
                                </div>
                              </td>
                            </tr>
                            {mMissed.map((m) => {
            ...
            <Card style={{ marginBottom: 16, borderLeft: `3px solid ${C.amber}` }}>
              <Sec sub="基于三个月观察提炼">季度复盘</Sec>
              <textarea
                value={insight}
                onChange={(e) => setInsight(e.target.value)}
                placeholder="本季整体交易行为总结、跨月规律、下季重点改进方向..."
                rows={5}
                style={{
