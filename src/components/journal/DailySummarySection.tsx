...
function parseEvalNote(plan: MnqDailyPlan, key: string): string {
  try {
    const raw = (plan as unknown as { evalNotesJson?: string | null })
      .evalNotesJson;
    if (!raw) return "";
  ...
  return (
    <div className="space-y-0.5">
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-foreground/80 min-w-0 flex-1 text-xs">
          {label}
        </span>
        {evalValue === true && (
          <span
        ...
        {evalValue == null && (
          <span className="text-muted-foreground text-[11px]">未评估</span>
        )}
      </div>
      {evalValue === false && note && (
        <p className="text-foreground/60 pl-2 text-xs leading-relaxed whitespace-pre-wrap">
          {note}
        </p>
      )}
    </div>
...
export function DailySummarySection({ session }: Props) {
  const hasAnyData =
    ...
    [session.marketContext, session.preMarketPlan, session.postReview].some(
      Boolean,
    );
    ...
    return (
      <Card className="border-dashed">
        <CardContent className="py-12 text-center">
          <p className="text-muted-foreground text-sm">今日尚无记录</p>
          <p className="text-muted-foreground mt-1 text-xs">
            在「盘前计划」中添加 Setup 开始记录
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span className="text-muted-foreground text-xs font-normal">
              ...
              {session.setups.filter((s) => s.status === "EXECUTED").length >
                0 && (
              ...
              {session.setups.filter((s) => s.status === "MISSED").length >
                0 && (
          ...
          {session.setups.map((setup) => (
            <SetupCard
              ...
              summaryMode={true}
              screenshots={
                setup.screenshots.length > 0 ? setup.screenshots : undefined
              }
      ...
      {session.mnqPlan?.scenario === "RANGE_SWEEP" &&
        [session.mnqPlan.sweepUpBand, session.mnqPlan.sweepDownBand].some(
          Boolean,
        ) && (
          <Card>
            <CardContent className="space-y-2 pt-4 pb-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold tracking-wide text-amber-500/90 uppercase">
                  MNQ 边界评估
                </span>
                <Badge
                  variant="outline"
                  className="border-amber-600/50 py-0 text-xs text-amber-400"
                >
                  震荡日
                </Badge>
              </div>
            ...
            <CardContent className="pt-4 pb-4">
              ...
              {session.mnqPlan.heldOvernight &&
                session.mnqPlan.overnightNote && (
                  <p className="text-foreground/75 mt-2 pl-5 text-xs leading-relaxed whitespace-pre-wrap">
                    {session.mnqPlan.overnightNote}
                  </p>
                )}
            </CardContent>
          ...
          <CardContent className="space-y-3 pt-4 pb-4">
            ...
            {(session.whatWentWell ?? session.lessonsLearned) && (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {session.whatWentWell && (
                  <div>
                    <p className="mb-0.5 text-xs font-medium text-green-400">
                      做对了
                    </p>
                    <p className="text-foreground/80 text-xs leading-relaxed whitespace-pre-wrap">
                      {session.whatWentWell}
                ...
                {session.lessonsLearned && (
                  <div>
                    <p className="mb-0.5 text-xs font-medium text-orange-400">
                      今日教训
                    </p>
                    <p className="text-foreground/80 text-xs leading-relaxed whitespace-pre-wrap">
                      {session.lessonsLearned}
                ...
                {session.planFollowed && (
                  <div>
                    ...
                    <span className="text-yellow-400">
                      {"★".repeat(session.planFollowed)}
                      {"☆".repeat(5 - session.planFollowed)}
                    </span>
                  </div>
                )}
                {session.emotionRating && (
                  <div>
                    ...
                    <span className="text-yellow-400">
                      {"★".repeat(session.emotionRating)}
                      {"☆".repeat(5 - session.emotionRating)}
                    </span>
                  </div>
                )}
                {session.focusRating && (
