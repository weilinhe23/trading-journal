...
interface CustomTooltipProps {
  active?: boolean
  payload?: Array<{ value?: number; payload?: StrategyItem }>
}
...
function CustomTooltip({ active, payload }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-lg border bg-popover p-2 text-xs shadow-md space-y-0.5">
      ...
      <p className="text-muted-foreground">{item.count} 笔 · 胜率 {item.winRate.toFixed(1)}%</p>
...
export function StrategyPnLChart({ data }: Props) {
  if (data.length === 0) {
    return (
